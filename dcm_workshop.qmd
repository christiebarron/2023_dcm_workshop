...
We can compare the AIC, BIC, and wald test or LRT to choose the best model.
...
#### Model Calibration Miscellaneous Information

[@ravand2020] provides a more detailed description of the Wald process of model calibration:

> the Wald statistic for the reduced models for individual items is calculated. The null hypotheses are the fit of the reduced model equals the fit of the general model. If the null hypothesis is rejected (p \<.05) the reduced model is rejected. If more than one reduced models are retained and the DINA or DINO are among the retained models, the DINA or DINO with the largest p value are selected, but if the DINA and DINO are not among the retained models, the reduced model with the largest p value is selected (p. 45).
...
Note we relied on reliative fit indices in model calibration as absolute fit indices are not suitable for model or Q-matrix selection for model comparison purposes[@ravand2020].
...
There are a variety of statistics to assess test-level absolute fit. The table below provides a brief summary of interpretation as articulated by [@shi2021].
...
|           | `> .05`: poor fit                                                                           |                                                                                                                                                                      |
+-----------+---------------------------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| MaxAD.r   | `adj p > .05`: good fit                                                                     | The maximum absolute difference between observed and predicted Fisher transformed correlations (MaxAD.r). Item-level fit & aggregated.                               |
|           |                                                                                             |                                                                                                                                                                      |
...
[@ravand2020] provides additional descriptions of model fit:

-   RMSEA: "root mean square error (RMSEA) is the mean difference between response proportions predicted by the model and those observed for each response category within each latent class weighted by the proportion of the test takers within the respective latent class."
-   SRMR: "standardized root mean squared residual (SRMSR) is a fit index borrowed from factor analysis. For any item pair, SRMSR is the observed correlation between the items minus the expected correlation"
-   Mean absolute distance for item-pair correlations: "mean absolute difference for the item-pair correlations (MADcor) statistic (DiBello et al., 2007) is also a measure of \[local dependence\], which is averaged across all the item correlation residuals: the difference between the observed and the model-predicted item correlations."
...
DCM requires substantial sample sizes. More attributes and more interaction effects requires larger sample sizes. [@ravand2020] references simulation studies suggesting even with a sample size of 1000, items with 1-2 attributes will have unreliable 2-way interaction effects. 3-way interactions are often unreliably estimated even with a sample size of 10,000. Sample size also has the largest impact on accurate q-matrix validation and model calibration.

**Item sample size**

"Hartz (2002) recommended at least three items for any given attribute. However, Jang (2009) suggested at least five items per attribute." [@ravand2020 p. 47]
