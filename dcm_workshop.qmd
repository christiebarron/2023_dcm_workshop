...
-   Introductory DCM
    ...
    -   Running DCM in R
    -   Interpreting parameter estimates of DCM models
    -   saving relevant output
-   Novel Advancements in DCM
    -   Generalized Models
    -   Item-specific modeling
    -   Classification reliability
    -   Empirical q-matrix development
-   Next Steps
    -   Advanced DCM applications (Hierarchical DCM, Longitudinal DCM, Polytomous DCM)
...
-   Discrite var result in multi-faceted classification OF LEARNERS ORGANIZED INTO LATENT CALSSES (UNLIKE MULTIDIMENSIONAL irt)

-   Item designed to measure 1+ latent attribues; exact design captured in Q-matrix w/ 0s and 1s (similar to CFA)

-   Response prob are class-specific and depend on attrib that mastered in partic class (probabilities depend on model)

-   Reliability estimates for discrete score variabales that yield the classifications can still be computed -- diff formula from IRT though
...
**==NOTE TO SELF MAYBE JUST USE OWN DATA HERE** ==![](fig/output-indiv-person.png) (reprinted from [@carragher2019])
...
Each model has certain conceptual properties:

+-----------+---------------+----------------------------------+----------------------+----------------+
| Model     | Link Function | Outcome                          | Attribute Assumption | Strategy       |
+===========+:=============:+:================================:+:====================:+:==============:+
| DINA      | Identity Link | Response Probability             | Non-proficient       | Non-proficient |
+-----------+---------------+----------------------------------+----------------------+----------------+
| DINO      | Identity Link | Response Probability             | Non-proficient       | Non-proficient |
+-----------+---------------+----------------------------------+----------------------+----------------+
| A-CDM     | Identity Link | Response Probability             | Proficient           | Proficient     |
+-----------+---------------+----------------------------------+----------------------+----------------+
| LLM/C-RUM | Logit Link    | Log-odds of Response Probability | Proficient           | Proficient     |
+-----------+---------------+----------------------------------+----------------------+----------------+
| R-RUM     | Log Link      | Log of Response Probability      | Proficient           | Proficient     |
+-----------+---------------+----------------------------------+----------------------+----------------+

They also each have statistical properties:

| Model     | Link Function |             Outcome              |
|-----------|:-------------:|:--------------------------------:|
| DINA      | Identity Link |       Response Probability       |
| DINO      | Identity Link |       Response Probability       |
| A-CDM     | Identity Link |       Response Probability       |
| LLM/C-RUM |  Logit Link   | Log-odds of Response Probability |
| R-RUM     |   Log Link    |   Log of Response Probability    |

#### Plots of Restricted DCM models

==add graphs for each of these==

-   DINA is characterized by

-   DINO is characteried by

-   A-CDM is characterized by

-   LLM/C-RUM is characterized by

-   R-RUM is characterized by
...
-   Imagine we want to run multiple regression model with three dichotomous indicator variables (i.e., dummy-coded).
...
-   Conceptually, General DCMs are the highest-order interaction model. If the interaction is 0, it simplifies down to simpler model.
    ...
    -   We value parsimony, and don't want to estimate parameters that aren't necessary. So if the interaction is 0, we want to use a simpler model. This will make more sense later.
...
+--------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------+--------------------------+
| [cdmTools](https://cran.r-project.org/web//packages/cdmTools/index.html) | Estimate small-sample DCM models, empirical q-matrix estimation, empirical number of attributes calculation | Proficient               |
+--------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------+--------------------------+
| Serena                                                                   | Proficient                                                                                                  | Proficient               |
+--------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------+--------------------------+

Many other software options too. Mplus can handle DCM with a lot of tedious manual coding, as can SAS. The [tdcm r package]() is great with longitudinal DCM.

==to remove==: cdmTools: Empirically estimate (with the discrete factor loading method) and validate (with the Hull method) the Q-matrix. Empirically assess the number of attributes in DCM applications using parallel analysis and the model-fit comparison approach. Estimate the general nonparametric classification method (GNPC). Estimate the restricted DINA model (R-DINA), a novel one-parameter model that is suitable for small-scale assessments, providing accurate attribute profile classifications and reliability estimates.
...
George and Robitzsch (@george2015) provide the following details about ECPE:
...
+-----------+----------------+--------------------------------------------------------------------------------------+----------------+----------------+
| Attribute | Description    | Parameter                                                                            | Skill          | Description    |
+===========+:==============:+:====================================================================================:+:==============:+:==============:+
| α1        | Non-proficient | Non-proficient                                                                       | Non-proficient | Non-proficient |
+-----------+----------------+--------------------------------------------------------------------------------------+----------------+----------------+
| α1        |                | word formation; combination of words into larger units such as phrases and sentences | Non-proficient | Non-proficient |
+-----------+----------------+--------------------------------------------------------------------------------------+----------------+----------------+
| Eunice    | Proficient     | Proficient                                                                           | Proficient     | Proficient     |
+-----------+----------------+--------------------------------------------------------------------------------------+----------------+----------------+
| Serena    | Proficient     | Proficient                                                                           | Proficient     | Proficient     |
...
|  \| \| \| \| \|-----------\|:-------------:\|:--------------------------------:\| \|α2 \| cohesive rules \| grammatical and lexical linking within a text or sentence \| \| α3 \| lexical rules \| modification of argument structures of lexical text elements (i. e., verbs and declensions) \| reprinted from (@george2015 p. 191)
...
==figure out how to get this work work==
...
(1) Empirically refine the Q-matrix using the PVAF method based on .95 cutoff.
(2) Fit the data using the G-DINA model and DINA model. Can the the DINA model fit the data as well as the G-DINA model? Use a Wald test and information criterion to evaluate it.

### Parameter Interpretation and Usage

(5) Find the estimated attribute profile for student #164. What are the estimated response probabilities for the latent classes for this student? Which attributes has the student most likely mastered or not mastered? Which attributes are we most certain about and for which attributes would we require more data to be certain for this student?
(6) Find the estimated item parameters for item #1. What are the diagnostic properties of this item? Is this an item with desirable measurement properties and would you recommend it for inclusion "as-is", potential revision, or potential removal from the pool?
(7) Find the estimated latent class membership probabilities for the cohort of students. What does that tell you about the skill profiles of the students?
(8) Find the estimated attribute mastery rates for the cohort of students. How do these mastery rates align with the skill profile information from the previous question?
(9) Create a diagnostic score report for student #20 (e.g., to provide guidance to the student for how to improve) as well as for the cohort of students overall (e.g., to provide instructional guidance to the teacher). Below are two sample templates that you can use for this purpose.
(10) What other kinds of materials would be valuable to have to give to the student or the teacher so that the recommendations that you made in the previous questions are truly actionable in practice?
